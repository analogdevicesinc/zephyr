 ...
 * @return 0 in case of success, negative error code otherwise.
 */
int adxl367_set_output_rate(const struct device *dev,
...
static int adxl367_probe(const struct device *dev)
{
	...
	data->act_proc_mode = cfg->act_proc_mode;
...
#define ADXL367_CONFIG(inst)								\
		...
		.th_mode = ADXL367_INSTANT_ON_LOW_TH,					\
		.autosleep = false,							\
		.low_noise = false,							\
		.temp_en = true,							\
		.range = ADXL367_2G_RANGE,						\
		.act_proc_mode = ADXL367_LOOPED,					\
		.activity_th.value = CONFIG_ADXL367_ACTIVITY_THRESHOLD / 100,		\
		...
		.inactivity_th.value = CONFIG_ADXL367_INACTIVITY_THRESHOLD / 100,	\
		.inactivity_th.referenced =						\
			IS_ENABLED(CONFIG_ADXL367_REFERENCED_ACTIVITY_DETECTION_MODE),	\
		.inactivity_th.enable = 1,						\
		.inactivity_time = CONFIG_ADXL367_INACTIVITY_TIME,			\
		.filter_settle = ADXL367_FILTER_SETTLE_370,				\
