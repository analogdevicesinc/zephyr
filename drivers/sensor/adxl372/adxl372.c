...
/* Exclude until we have a API to better support this */
#if 0
/**
 ...
 * @return 0 in case of success, negative error code otherwise.
 */
static int adxl372_get_fifo_xyz_data(struct device *dev,
				     ...
				     u16_t cnt)
{
	...
	do {
		...
		ret = adxl372_reg_read_multiple(dev, ADXL372_FIFO_DATA,
						buf, num);

		for (i = 0; i < num; i += 6) {
			samples->x = ((buf[i] << 8) | (buf[i+1] & 0xF0));
 ...
 * @return 0 in case of success, negative error code otherwise.
 */
int adxl372_service_fifo_ev(struct device *dev,
				...
				u16_t *fifo_entries)
{
	...
	if (ADXL372_STATUS_1_FIFO_OVR(status1)) {
		SYS_LOG_ERR("FIFO overrun");
		return -EFAULT;
	}

	if (data->fifo_config.fifo_mode != ADXL372_FIFO_BYPASSED) {
