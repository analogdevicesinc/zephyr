...
static uint32_t last_announced_cyc;
...
static void max32_wut_timer_isr(const void *arg)
{
	...
	uint32_t delta_cyc = curr - last_announced_cyc;
	uint32_t delta_ticks = delta_cyc / CYC_PER_TICK;

	last_announced_cyc += delta_ticks * CYC_PER_TICK;

#if !IS_ENABLED(CONFIG_TICKLESS_KERNEL)
	/* Set up next compare for non-tickless mode */
	uint32_t next_cyc = last_announced_cyc + CYC_PER_TICK;

	if ((next_cyc - curr) < MIN_DELAY_CYC) {
		next_cyc = curr + MIN_DELAY_CYC;
	}
...
void sys_clock_set_timeout(int32_t ticks, bool idle)
{
	...
	MXC_WUT_Disable(WUT_REGS);
	uint32_t curr = MXC_WUT_GetCount(WUT_REGS);
	uint32_t unannounced_cyc = curr - last_announced_cyc;
	uint32_t unannounced_ticks = unannounced_cyc / CYC_PER_TICK;

	/* Calculate next compare value aligned to tick boundary */
	uint32_t next_ticks = unannounced_ticks + ticks + 1;

	if (next_ticks > MAX_TICKS) {
		next_ticks = MAX_TICKS;
	}

	uint32_t next_cyc = last_announced_cyc + (next_ticks * CYC_PER_TICK);

	/* Ensure we have enough time before the compare interrupt */
	if ((next_cyc - curr) < MIN_DELAY_CYC) {
		next_cyc = curr + MIN_DELAY_CYC;
	}
