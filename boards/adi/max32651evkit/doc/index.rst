...
To sign the firmware image, user needs to enable ``CONFIG_MAX32_SECURE_SOC`` Kconfig option and specify
the path to the private key using ``CONFIG_MAX32_SECURE_SOC_KEY_PATH`` Kconfig option. Once these are
configured, firmware signing is performed automatically in the background during the ``west build``
process.
